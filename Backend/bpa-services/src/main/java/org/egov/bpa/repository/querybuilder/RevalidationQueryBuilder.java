...
@Component
public class RevalidationQueryBuilder {
	...
	private static final String QUERY = "SELECT ebpr.id as ebpr_id,ebpr.isSujogExistingApplication as ebpr_isSujogExistingApplication,ebpr.tenantid as ebpr_tenantId,ebpr.bpaApplicationNo as ebpr_bpaApplicationNo,ebpr.bpaApplicationId as ebpr_bpaApplicationId,ebpr.refBpaApplicationNo as ebpr_refBpaApplicationNo,ebpr.permitNo as ebpr_permitNo,ebpr.permitDate as ebpr_permitDate,ebpr.permitExpiryDate as ebpr_permitExpiryDate,ebpr.refApplicationDetails as ebpr_refApplicationDetails,ebpr.createdby as ebpr_createdBy,ebpr.lastmodifiedby as ebpr_lastModifiedBy,ebpr.createdtime as ebpr_createdTime,ebpr.lastmodifiedtime as ebpr_lastModifiedTime, isConstructionPresent, isConstructionAsPerApprovedPlan"
			+ ", ebprd.id as ebprd_id, ebprd.documenttype as ebprd_documenttype, ebprd.filestoreid as ebprd_filestoreid, ebprd.documentuid as ebprd_documentuid, ebprd.revalidationId as ebprd_revalidationId, ebprd.additionaldetails as ebprd_additionaldetails, ebprd.createdby as ebprd_createdby, ebprd.lastmodifiedby as ebprd_lastmodifiedby, ebprd.createdtime as ebprd_createdtime, ebprd.lastmodifiedtime as ebprd_lastmodifiedtime "
			+ " FROM eg_bpa_buildingplan_revalidation ebpr "
			+  LEFT_OUTER_JOIN_STRING
			+ " eg_bpa_revalidation_documents ebprd on ebprd.revalidationId=ebpr.id"
	        +  LEFT_OUTER_JOIN_STRING
			+ " eg_bpa_buildingplan bpa on bpa.applicationno=ebpr.refbpaapplicationno";
	 ...
	 *
	 */
	public String getrevalidationSearchQuery(RevalidationSearchCriteria criteria, List<Object> preparedStmtList) {
		...
		if (criteria.getPermitNo() != null) {
			addClauseIfRequired(preparedStmtList, builder);
			builder.append(" ebpr.permitNo=? ");
			preparedStmtList.add(criteria.getPermitNo());
		}
		if (criteria.getPermitDate() != null) {
			addClauseIfRequired(preparedStmtList, builder);
			builder.append(" ebpr.permitDate=? ");
			preparedStmtList.add(criteria.getPermitDate());
		}
		if (criteria.getPermitExpiryDate() != null) {
			addClauseIfRequired(preparedStmtList, builder);
			builder.append(" ebpr.permitExpiryDate=? ");
			preparedStmtList.add(criteria.getPermitExpiryDate());
		}
	...
	private String addPaginationWrapper(String query, List<Object> preparedStmtList,
			RevalidationSearchCriteria criteria) {
