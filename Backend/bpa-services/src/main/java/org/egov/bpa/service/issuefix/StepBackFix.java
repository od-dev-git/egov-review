...
@Service("stepBackToApprovalInprogress")
@Slf4j
public class StepBackFix implements IIssueFixService {
	...
	public IssueFix issueFix(IssueFixRequest issueFixRequest) {
		...
		if (!bpa.get(0).getStatus().equalsIgnoreCase(IssueFixConstants.PENDING_SANC_FEE)) {
			throw new CustomException("APPLICATION_STATUS_ERROR",
					"Application is not at Sanc Fee Pending Payment Stage. Can't revert to 1 step back !!");
		}
		...
		Installment installmentToBeDeleted = checkIfInstallmentToBeDeleted(issueFixRequest, isDataUpdateNeeded);

		DscDetails dscToBeDeleted = checkIfDscToBeDeleted(issueFixRequest, isDataUpdateNeeded, searchCriteria);
		DscDetails planDscToBeDeleted = checkIfPlanDscToBeDeleted(issueFixRequest, isDataUpdateNeeded, searchCriteria);

		updateDataIfNeeded(demandToBeUpdated, processInstance, applicationToBeUpdated, isPaymentReceived,
				isDataUpdateNeeded);

		return issueFixRequest.getIssueFix();
	}

	private DscDetails checkIfPlanDscToBeDeleted(IssueFixRequest issueFixRequest,
			Map<String, Boolean> isDataUpdateNeeded, BPASearchCriteria searchCriteria) {
	...
	private void updateDataIfNeeded(Demand demandToBeDeleted, ProcessInstance processInstance,
		...
		if (isDataUpdateNeeded.containsKey(IssueFixConstants.IS_PLAN_DSC_DELETE_NEEDED)
			...
			repository.deletePlanDsc(applicationToBeUpdated);
		}
	...
	private Demand checkIfDemandUpdatedProperly(IssueFixRequest issueFixRequest,
		...
		if (demands.size() > 1) {
			throw new CustomException("MULTIPLE_DEMNADS_FOUND",
					"Multiple Sanc Fee demand found for application no :" + applicationNumber);
			...
			if (demands.get(0).getIsPaymentCompleted().equals(Boolean.TRUE)) {
				throw new CustomException("PAYMENT_DONE_FOR_DEMAND",
						"Payment has been done for sanc fee demand application no :" + applicationNumber);
			} else {
	...
	private Boolean checkIfPaymentReceived(IssueFixRequest issueFixRequest) {
		...
		if (payments.size() >= 1) {
			...
			throw new CustomException("PAYMENT_ISSUE",
					"Sanc fee payment has been done for application no :" + applicationNumber + " Can't step back !");
		}
