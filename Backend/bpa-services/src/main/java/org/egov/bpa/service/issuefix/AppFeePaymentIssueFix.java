...
@Service("appFeePaymentIssueFixService")
@Slf4j
public class AppFeePaymentIssueFix implements IIssueFixService {
	...
	public IssueFix issueFix(IssueFixRequest issueFixRequest) {
		...
		if (!bpa.get(0).getStatus().equalsIgnoreCase(IssueFixConstants.PENDING_APPLICATION_FEE)) {
	...
	private void updateDataIfNeeded(Demand demandToBeUpdated, ProcessInstance processInstance,
			BPA applicationToBeUpdated, Payment payment, Map<String, Boolean> isDataUpdateNeeded) {
	...
	private BPA checkIfApplicationUpdatedProperly(IssueFixRequest issueFixRequest, BPA bpa,
		...
		if (bpa.getStatus().equalsIgnoreCase(IssueFixConstants.BPA_DOC_VERIFICATION_INPROGRESS)) {
