...
@Service("regAppFeePaymentIssueFixService")
@Slf4j
public class RegularizationAppFeePaymentIssueFix implements RegularizationIIssueFixService {
	...
	public IssueFix issueFix(IssueFixRequest issueFixRequest) {
		...
		if (!regularizations.get(0).getStatus().equalsIgnoreCase(IssueFixConstants.PENDING_APPLICATION_FEE)) {
			throw new CustomException("APPLICATION_STATUS_ERROR", "Application is not at Pending Payment Stage. Kindly Check !!");
		}
		...
		return IssueFix.builder().applicationNo(applicationNo).tenantId(tenantId).build();
	}
	...
	private void updateDataIfNeeded(Demand demandToBeUpdated, ProcessInstance processInstance,
			Regularization applicationToBeUpdated, Payment payment, Map<String, Boolean> isDataUpdateNeeded) {
		...
		if (isDataUpdateNeeded.containsKey(IssueFixConstants.IS_APPLICATION_UPDATE_NEEDED)
			...
			repository.updateApplication(applicationToBeUpdated, payment);
		}
		if (isDataUpdateNeeded.containsKey(IssueFixConstants.IS_WORKFLOW_UPDATE_NEEDED)
			...
			repository.updateWorkflow(processInstance, payment);
		}
	...
	private Regularization checkIfApplicationUpdatedProperly(IssueFixRequest issueFixRequest, Regularization regularization,
		...
		if (regularization.getStatus().equalsIgnoreCase(IssueFixConstants.REG_DOC_VERIFICATION_INPROGRESS)) {
	...
	private ProcessInstance checkIfWorkFlowUpdatedProperly(IssueFixRequest issueFixRequest,
		...
		List<ProcessInstance> processInstanceForApply = processInstances.stream()
				.filter(process -> process.getAction().equalsIgnoreCase(IssueFixConstants.APPLY))
	...
	private Demand checkIfDemandUpdatedProperly(IssueFixRequest issueFixRequest,
		...
		DemandSearchCriteria demandSearchCriteria = DemandSearchCriteria.builder().consumerCode(applicationNumber)
				.businessService(IssueFixConstants.REG_APP_FEE).tenantId(tenantId).build();
