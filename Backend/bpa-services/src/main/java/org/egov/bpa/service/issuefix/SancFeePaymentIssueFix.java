...
@Service("sancFeePaymentIssueFixService")
@Slf4j
public class SancFeePaymentIssueFix implements IIssueFixService {
	...
	public IssueFix issueFix(IssueFixRequest issueFixRequest) {
		...
		if (!bpa.get(0).getStatus().equalsIgnoreCase(IssueFixConstants.PENDING_SANC_FEE)) {
			throw new CustomException("APPLICATION_STATUS_ERROR",
					"Application is not at Sanction Fee Pending Payment Stage. Kindly Check !!");
		}
	...
	private void updateDataIfNeeded(Demand demandToBeUpdated, ProcessInstance processInstance,
			BPA applicationToBeUpdated, Payment payment, Map<String, Boolean> isDataUpdateNeeded, RequestInfo requestInfo) {
	...
	private BPA checkIfApplicationUpdatedProperly(IssueFixRequest issueFixRequest, BPA bpa,
		...
		if (bpa.getStatus().equalsIgnoreCase(IssueFixConstants.APPROVED)) {
	...
	private ProcessInstance checkIfWorkFlowUpdatedProperly(IssueFixRequest issueFixRequest,
		...
		List<ProcessInstance> processInstanceForApply = processInstances.stream()
			...
			//	.sorted(Comparator.comparing(ProcessInstance::getAuditDetails.get).reversed())
...
//			repository.updateDemand(demand);
	 ...
	 * @param issueFixRequest
	 */
	private Payment checkIfPaymentReceivedForSancFee(IssueFixRequest issueFixRequest) {
