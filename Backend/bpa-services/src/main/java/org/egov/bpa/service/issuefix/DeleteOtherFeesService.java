...
@Service("deleteOtherFeesService")
@Slf4j

public class DeleteOtherFeesService implements IIssueFixService {
	...
	public IssueFix issueFix(IssueFixRequest issueFixRequest) {
		...
		if(!isPaymentReceived) {
			...
			throw new CustomException("PAYMENT_ISSUE", "Saction fee payment has been done for application no :" + applicationNo + " Can't delete mentioned application number !");
		}
	...
	private void updateApplicationDetails(IssueFixRequest issueFixRequest) {
		issueFixRepository.deleteOtherFeeFromAdditionalDetails(issueFixRequest.getIssueFix());
	}
