...
public class ReceiptServiceV2 {
	...
	private void updateSingleDemandDetail(DemandDetail currentDetail, BillAccountDetailV2 billAccDetail,
		...
		BigDecimal currentDemandTax = currentDetail.getTaxAmount();
		BigDecimal taxFromBillAccDetail = billAccDetail.getAmount(); 
		...
		if(advanceTaxHead!=null && billAccDetail.getTaxHeadCode().equalsIgnoreCase(advanceTaxHead)){
			...
			//if receipt is cancelled, advance will be subtracted here
			if(isRecieptCancellation) currentDetail.setTaxAmount(currentDemandTax.subtract(taxFromBillAccDetail));
		}
