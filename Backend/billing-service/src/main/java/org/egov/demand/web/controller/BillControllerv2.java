...
import org.egov.demand.custom.BillCriteria;
import org.egov.demand.custom.CustomBillResponse;
import org.egov.demand.custom.CustomBillService;
...
public class BillControllerv2 {
	...
	@PostMapping("_customfetchbill")
	@ResponseBody
	public ResponseEntity<?> customFetchBill(@RequestBody RequestInfoWrapper requestInfoWrapper, 
			@ModelAttribute @Valid BillCriteria billCriteria){
		
		CustomBillResponse billResponse = customBillService.fetchBill(billCriteria, requestInfoWrapper);
		return new ResponseEntity<>(billResponse, HttpStatus.CREATED);
	}
}
