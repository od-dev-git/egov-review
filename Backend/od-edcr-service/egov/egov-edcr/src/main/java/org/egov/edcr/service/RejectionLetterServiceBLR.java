...
@Service
public class RejectionLetterServiceBLR extends PermitOrderService {
	...
	public static String PARAGRAPH_1_1 = "Ref: : Sujog-OBPS application No. %s in respect of %s";
	...
	public InputStream createPdf(Plan plan, LinkedHashMap bpaApplication, RequestInfo requestInfo, Boolean isPreview)
		...
		String appType = getValue(bpaApplication, "appType");
		...
		String appTypeinSub = appType.equals("LAND") ? "Land Regularization." : "Building Regularization.";

		subject = String.format(PARAGRAPH_1_1, applicationNo, appTypeinSub);
		...
		Image qrCode = getQrCode("", applicationNo, approvalDate,
				getValue(bpaApplication, "applicationType"));
		...
		addRowsForPlotTableBLR(requestInfo, plotTable, bpaApplication, tenantIdActual);
		document.add(Chunk.NEWLINE);
	...
	public void addRowsForPlotTableBLR(RequestInfo reqInfo, PdfPTable table1, LinkedHashMap bpaApplication,
			String tenantId) {
		java.util.List<Map<String, Object>> plotList = getPlotAndKhataDetailsBLR(bpaApplication);
		...
		if (!CollectionUtils.isEmpty(plotList)) {
			for (Map<String, Object> plots : plotList) {
				...
				Phrase gpaHolderNamePhrase = new Phrase(
						plots.get("gpaHoldername") != null ? plots.get("gpaHoldername") + "" : "NA", fontPara1);
	...
	public java.util.List<Map<String, Object>> getPlotAndKhataDetailsBLR(LinkedHashMap bpaApplication) {

		java.util.List<Map<String, Object>> plotList = new ArrayList<>();

		if (Objects.nonNull(bpaApplication) && Objects.nonNull(bpaApplication.get("landRegularizationInfo"))
				&& bpaApplication.get("landRegularizationInfo") instanceof java.util.Map
				&& !CollectionUtils.isEmpty((java.util.Map) bpaApplication.get("landRegularizationInfo"))
				&& !CollectionUtils
						.isEmpty((java.util.List) ((java.util.Map) bpaApplication.get("landRegularizationInfo"))
								.get("plotInfo"))) {

			plotList = (java.util.List) ((java.util.Map) bpaApplication.get("landRegularizationInfo")).get("plotInfo");
		}
