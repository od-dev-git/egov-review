...
//                        + applicationDetail.getDcrNumber());
		...
		for (org.egov.edcr.entity.EdcrPdfDetail planPdf : applicationDetail.getEdcrPdfDetails()) {
			
			if (planPdf.getLayer().equalsIgnoreCase("BASE_LAYERS")) {
				if (planPdf.getConvertedPdf() != null) {			
					applicationInfo.setBaseLayerFileStoreId(planPdf.getConvertedPdf().getFileStoreId());
				}
			}
