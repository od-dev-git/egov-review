...
@Service
public class PermitOrderServiceBPA6 extends PermitOrderService {

private static final Logger LOG = Logger.getLogger(PermitOrderServiceBPA6.class);
...
//	public static String PARAGRAPH_2_11_3_A = "Sanction fee : Rs 6,689.00/-only";
	...
	public InputStream generateReport(Plan plan, LinkedHashMap bpaApplication, RequestInfo requestInfo, Boolean isPreview) {
		try {
			...
			PlanPreApproved  plans= bpaService.loadPreApprovedData(edcrNo,bpaApplication,requestInfo);
			return createPdf(plans, bpaApplication, requestInfo, isPreview);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public InputStream createPdf(PlanPreApproved plan, LinkedHashMap bpaApplication, RequestInfo requestInfo, Boolean isPreview) throws Exception {
		
		String tenantIdActual = getValue(bpaApplication, "tenantId");
		
		OdishaUlbs ulb = OdishaUlbs.getUlb(tenantIdActual);
		
		//System.out.println("tenantid"+tenantIdActual);
		...
		Chunk forServiceType = new Chunk(String.format("For %s of a ", bpaService.getServiceTypeBPA6(plan)), font1);
		String floorInfo = plan.getFloorInfo() + " ";
		Chunk floorInform = new Chunk(floorInfo, fontBold);
		Chunk storeyed = new Chunk(PARAGRAPH_1_3, font1);
		String subOccupancy = plan.getSubOccupancy() + " ";
		...
		Phrase paragraph2New = new Phrase();
		Phrase paragraph2New2 = new Phrase();
		...
		if(CollectionUtils.isEmpty(getPlotAndKhataDetails(additionalDetails))) {
			...
			//table
			paragraph2New.add(forServiceType);
			paragraph2New.add(floorInform);
			paragraph2New.add(storeyed);
			paragraph2New.add(subOccupanc);
			paragraph2New.add(buildingInRespectOf);
			paragraph2New2.add(village);
			paragraph2New2.add(locality);
			paragraph2New2.add(withinDevelopmentPlanArea);
			
		}
		
		Paragraph secondPara = new Paragraph(paragraph2);
		Paragraph secondParaNew = new Paragraph(paragraph2New);
		Paragraph secondParaNew2 = new Paragraph(paragraph2New2);
	...
	//	list1Item3.add(chunk18);
		...
		Chunk chunk30 = new Chunk(
				plan.getFloorInfo() + " " + plan.getSubOccupancy() + " ",
				fontBold);
		...
		if (isPreview) {
			...
			java.util.List<Map<String, Object>> estimatedFeeDetails = getEstimatedSanctionFeeDetails(requestInfo, bpaApplication);
		...
		BigDecimal plotAreaAsPerDeclaration = plan.getPlotArea().setScale(2, BigDecimal.ROUND_UP);
		...
		Chunk chunk42 = new Chunk(plan.getTotalBuitUpArea()!= null
				? plan.getTotalBuitUpArea().setScale(2, BigDecimal.ROUND_UP) + SQM
				: "0", fontBold);
		...
		BigDecimal providedFar = BigDecimal.valueOf(plan.getProvidedFar());
		Chunk chunk45 = new Chunk(
				plan.getTotalfloorArea().setScale(2, BigDecimal.ROUND_UP) + SQM,
				fontBold);
		...
		Chunk chunk48 = new Chunk(plan.getProvidedFar() + "", fontBold);
		...
		Paragraph NA_noc = new Paragraph("NA", fontBold);
		...
		Map<String, Object> approverDetailsMap = getApproverDetails(requestInfo, tenantId, "BPA6", approvedBy,
				"EMPLOYEE");
		...
		if (CollectionUtils.isEmpty(getPlotAndKhataDetails(additionalDetails))) {
			...
			// Location detail update enhancement : Existing location details to be removed
			// from scrutiny.
	...
	private void addRows1(PdfPTable table1, PlanPreApproved plan) {
	...
	//java.util.List<DcrReportBlockDetail> blockDetails = buildBlockWiseProposedInfo(plan);
		...
		java.util.List blocks = bpaService.getblockDetails();
		rowSpanHelper = blocks.size();
	...
	//	table1.addCell(cell8);
		...
		for (int i=0;i<blocks.size();i++) {
			...
			Phrase blockNamephrase = new Phrase("Block " + (++Blkno) + " Details", fontPara1);
			...
			//my code starts-
//			Map<String, BigDecimal> setBackData =edcrExternalService.getSetBackData( block);
			
			Map<String, String> setBackData = bpaService.getSetBackDatapreApproved(plan,blocks,i);
			String frontSetbackProvided = setBackData.get("frontSetbackRequired");
			String rearSetbackProvided = setBackData.get("rearSetbackRequired");
		    String leftSetbackProvided = setBackData.get("leftSetbackRequired");
			String rightSetbackProvided = setBackData.get("rightSetbackRequired");
//			LOG.info("frontSetbackProvided"+frontSetbackProvided);

		int noOfFloors=0;
			java.util.List floorDetails = bpaService.getfloordetails(plan,blocks,i);
			noOfFloors = floorDetails.size() + 1;
...
//			}
			...
			for (int k =0;k<floorDetails.size();k++) {
				...
				Map<String,String> floor =	bpaService.getfloorinfo(floorDetails,k);
				Phrase floorNamephrase = new Phrase(floor.get("floorName"), fontPara1);
				floorNameCell.addElement(floorNamephrase);
				table1.addCell(floorNameCell);
				PdfPCell floorAreaCell = new PdfPCell();
				Phrase floorAreaphrase = new Phrase(floor.get("builtUpArea") + "", fontPara1);
