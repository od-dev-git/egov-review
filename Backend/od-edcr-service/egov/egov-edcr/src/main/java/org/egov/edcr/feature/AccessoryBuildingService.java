...
public class AccessoryBuildingService extends FeatureProcess {
	...
	private static final String SUBRULE_88_1 = "";
	private static final String SUBULE_88_3 = "";
	private static final String SUBRULE_88_4 = "";
	private static final String SUBRULE_88_5 = "";
	...
	public Plan validate(Plan plan) {
		...
		if (plan != null && !plan.getAccessoryBlocks().isEmpty()) {
			...
			if (plan != null && plan.getPlot() != null && plan.getPlot().getArea() != null
					&& plan.getPlot().getArea().compareTo(BigDecimal.valueOf(0)) < 150) {
				errors.put("AccessoryBuildingService.plotArea",
						"Minimum plot area should be 150 " + DcrConstants.IN_METER_SQR);
			...
			if (!shortestDistanceDefined) {
				errors.put(DcrConstants.SHORTESTDISTANCETOROAD, DcrConstants.SHORTESTDISTANCETOROAD + " not defined ");
				plan.addErrors(errors);
			}
		}
	...
	public Plan process(Plan plan) {
		...
		processDistanceBetweenMainBuildingAndTheOutHouse(plan);

		return plan;
	}

	private void processDistanceBetweenMainBuildingAndTheOutHouse(Plan plan) {
		...
		if (plan != null && !plan.getAccessoryBlocks().isEmpty()) {
			for (AccessoryBlock accessoryBlock : plan.getAccessoryBlocks()) {
				...
				if (accessoryBlock.getAccessoryBuilding() != null
						...
						&& accessoryBlock.getAccessoryBuilding().getHeight().compareTo(BigDecimal.valueOf(0)) > 0) {
					if (accessoryBlock.getAccessoryBuilding().getHeight().compareTo(BigDecimal.valueOf(3)) <= 0) {
						valid = true;
					}
					if (valid) {

						setReportOutputDetails(plan, subRule, String.format(subRuleDesc, accessoryBlock.getNumber()),
								BigDecimal.valueOf(3) + DcrConstants.IN_METER,
	...
	private void processAreaOfAccessoryBlock(Plan plan) {
		...
		if (plan != null && plan.getPlot() != null && plan.getPlot().getArea() != null
			...
			BigDecimal maximumAllowedArea = new BigDecimal("30.00");
			if (!plan.getAccessoryBlocks().isEmpty()) {
				for (AccessoryBlock accessoryBlock : plan.getAccessoryBlocks()) {
					...
					Boolean valid = false;
					if (accessoryBlockArea != null && accessoryBlockArea.compareTo(BigDecimal.ZERO) > 0) {
						if (accessoryBlockArea.compareTo(maximumAllowedArea) <= 0) {
							valid = true;
						}
						if (valid) {
							...
							setReportOutputDetails(plan, subRule,
									...
									maximumAllowedArea + DcrConstants.IN_METER_SQR,
