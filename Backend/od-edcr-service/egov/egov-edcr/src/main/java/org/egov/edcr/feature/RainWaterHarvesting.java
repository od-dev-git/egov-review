...
public class RainWaterHarvesting extends FeatureProcess {
	...
	// private static final String RAINWATER_HARVESTING_TANK_CAPACITY = "Minimum
	// capacity of Rain Water Harvesting Tank";
	...
	public Plan process(Plan pl) {
		...
		String subRule = "ODA Rule 47(1,3)";
		...
		OccupancyTypeHelper mostRestrictiveFarHelper = pl.getVirtualBuilding() != null
				...
				: null;
		boolean isRainWaterRequired=false;
		boolean isRechargingPitsRequired=false;
		if (plotArea.compareTo(new BigDecimal("115")) >= 0) {
			isRainWaterRequired=true;
		}
		
		//Rain Water Harvesting
		
		if (!isRainWaterRequired) {
	...
	private void addReportOutput(Plan pl, String subRule, String subRuleDesc) {
		if (pl.getUtility() != null) {
			if (pl.getUtility().getRainWaterHarvest() != null && !pl.getUtility().getRainWaterHarvest().isEmpty()) {
				setReportOutputDetails(pl, subRule, subRuleDesc, null, "Defined in the plan",
						Result.Verify.getResultVal());
			} else {
				setReportOutputDetails(pl, subRule, subRuleDesc, null, "Not Defined in the plan",
						Result.Not_Accepted.getResultVal());
			}
		}
	}
	...
	/*
	 * private void processRWHTankCapacity(Plan plan, String rule, String subRule,
	 * String subRuleDesc, BigDecimal expectedTankCapacity, Boolean valid) { if
	 * (expectedTankCapacity.compareTo(BigDecimal.valueOf(0)) > 0) { if (valid) {
	 * setReportOutputDetails(plan, subRule, RAINWATER_HARVESTING_TANK_CAPACITY,
	 * expectedTankCapacity.toString(),
	 * plan.getUtility().getRainWaterHarvestingTankCapacity().toString(),
	 * Result.Accepted.getResultVal()); } else { setReportOutputDetails(plan,
	 * subRule, RAINWATER_HARVESTING_TANK_CAPACITY, expectedTankCapacity.toString()
	 * + IN_LITRE, plan.getUtility().getRainWaterHarvestingTankCapacity().toString()
	 * + IN_LITRE, Result.Not_Accepted.getResultVal()); } } }
	 */
	...
	/*
	 * private boolean validateRWH(Plan pl, HashMap<String, String> errors) { if
	 ...
	 * pl.addErrors(errors); return true; } else if
	 * (!pl.getUtility().getRainWaterHarvest().isEmpty() &&
	 ...
	 * pl.addErrors(errors); return true; } return false; }
	 */
