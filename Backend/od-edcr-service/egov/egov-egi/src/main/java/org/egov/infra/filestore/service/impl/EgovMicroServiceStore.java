...
public class EgovMicroServiceStore implements FileStoreService {
	...
	@Override
	public FileStoreMapper store(InputStream sourceFileStream, String fileName, String mimeType, String moduleName, String tenantId) {
		...
		try {
			...
			FileUtils.copyToFile(sourceFileStream, f);
...
//			map.add("file", new FileSystemResource(file.getName()));
			...
			LOG.debug(String.format("Uploaded file   %s   with filestoreid  %s " ,f.getName() , fileMapper.getFileStoreId()));
...
//				Files.deleteIfExists(Paths.get(fileName));
			...
			throw new CustomResourceAccessException("Error occurred while saving the file to filestore", e);
		}
