...
@Service
public class RegularizationDemandService {
	...
	/**
	 * Generate demands from billing service from this method
	 * 
	 * @param requestInfo
	 * @param calculations
	 */
	public void generateDemand(RequestInfo requestInfo, List<RegularizationCalculation> calculations) {

		// List that will contain Calculation for new demands
		List<RegularizationCalculation> createCalculations = new LinkedList<>();

		// List that will contain Calculation for old demands
		List<RegularizationCalculation> updateCalculations = new LinkedList<>();
		...
		if (!CollectionUtils.isEmpty(calculations)) {
			...
			Set<String> applicationNos = calculations.stream()
					.map(calculation -> calculation.getRegularization().getApplicationNo()).collect(Collectors.toSet());
//            Set<String> applicationNumbers = calculations.stream().map(calculation -> calculation.getBPA().getApplicationNo()).collect(Collectors.toSet());
			...
			for (RegularizationCalculation calculation : calculations) {
				if (!applicationNumbersFromDemands.contains(calculation.getRegularization().getApplicationNo()))
	...
	/**
	 * update demand from this method if needed
	 * 
	 * @param requestInfo
	 * @param calculations
	 * @return
	 */
	private List<Demand> updateDemand(RequestInfo requestInfo, List<RegularizationCalculation> calculations) {

		List<Demand> demands = new LinkedList<>();
		for (RegularizationCalculation calculation : calculations) {

			List<Demand> searchResult = searchDemand(calculation.getTenantId(),
					Collections.singleton(calculation.getRegularization().getApplicationNo()), requestInfo,
					calculation);

			if (CollectionUtils.isEmpty(searchResult))
				throw new CustomException(BPACalculatorConstants.INVALID_UPDATE,
						"No demand exists for applicationNumber: "
								+ calculation.getRegularization().getApplicationNo());
	...
	/**
	 * child method for update demand method
	 * 
	 * @param calculation
	 * @param demandDetails
	 * @return
	 */
	private List<DemandDetail> getUpdatedDemandDetails(RegularizationCalculation calculation,
	...
	/**
	 * Prepare request for demand generation from this method
	 * 
	 * @param requestInfo
	 * @param calculations
	 * @return
	 */
	private List<Demand> createDemand(RequestInfo requestInfo, List<RegularizationCalculation> calculations) {

		List<Demand> demands = new LinkedList<>();
		for (RegularizationCalculation calculation : calculations) {
			Regularization regularization = null;

			if (calculation.getRegularization() != null) {
				regularization = calculation.getRegularization();
			} else if (calculation.getApplicationNumber() != null) {
				regularization = regularizationService.getRegularization(requestInfo, calculation.getTenantId(),
						calculation.getApplicationNumber());
			}

			if (regularization == null)
				throw new CustomException(BPACalculatorConstants.INVALID_APPLICATION_NUMBER,
						"Demand cannot be generated for applicationNumber " + calculation.getApplicationNumber()
								+ "  Regularization application with this number does not exist ");

			String tenantId = calculation.getTenantId();
			String consumerCode = calculation.getRegularization().getApplicationNo();

			User owner = regularization.getOwners().get(0).toCommonUser();
			...
			demands.add(Demand.builder().consumerCode(consumerCode).demandDetails(demandDetails).payer(owner)
					.billExpiryTime(RegularizationConstants.BILL_EXPIRY_TIME)
					.minimumAmountPayable(config.getMinimumPayableAmount()).tenantId(tenantId)
					.taxPeriodFrom(startCal.getTimeInMillis()).taxPeriodTo(endCal.getTimeInMillis()).consumerType("BLR")
					.businessService(utils.getBillingBusinessService(regularization.getBusinessService(),
	...
	/**
	 * Search demands from this method
	 * 
	 * @param tenantId
	 * @param consumerCodes
	 * @param requestInfo
	...
	private List<Demand> searchDemand(String tenantId, Set<String> consumerCodes, RequestInfo requestInfo,
			RegularizationCalculation calculation) {

		String feeType = calculation.getFeeType();
		String uri = utils.getDemandSearchURL();
		uri = uri.replace("{1}", tenantId);
		uri = uri.replace("{2}",
				(utils.getBillingBusinessService(calculation.getRegularization().getBusinessService(), feeType)));
	...
	/**
	 * Add round offs in the demand details with this method
	 * 
	 * @param tenantId
	 * @param demandDetails
	 */
	private void addRoundOffTaxHead(String tenantId, List<DemandDetail> demandDetails) {
		...
		for (DemandDetail demandDetail : demandDetails) {
			/*
			 * if(!demandDetail.getTaxHeadMasterCode().equalsIgnoreCase(
			 * BPACalculatorConstants.MDMS_ROUNDOFF_TAXHEAD)) totalTax =
			 * totalTax.add(demandDetail.getTaxAmount()); else
			 */
		 ...
		 */
		/*
		 * if(prevRoundOffDemandDetail!=null){ roundOff =
		 * roundOff.subtract(prevRoundOffDemandDetail.getTaxAmount()); }
		 */

		if (roundOff.compareTo(BigDecimal.ZERO) != 0) {
			DemandDetail roundOffDemandDetail = DemandDetail.builder().taxAmount(roundOff)
					.taxHeadMasterCode(BPACalculatorConstants.MDMS_ROUNDOFF_TAXHEAD).tenantId(tenantId)
					.collectionAmount(BigDecimal.ZERO).build();
