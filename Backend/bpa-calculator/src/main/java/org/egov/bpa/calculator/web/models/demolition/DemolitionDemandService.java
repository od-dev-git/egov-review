...
import org.egov.bpa.calculator.services.DemolitionService;
...
@Service
public class DemolitionDemandService {
	 ...
	 * @param calculations
	 */
	public void generateDemand(RequestInfo requestInfo, List<DemolitionCalculation> calculations) {

		// List that will contain Calculation for new demands
		List<DemolitionCalculation> createCalculations = new LinkedList<>();

		// List that will contain Calculation for old demands
		List<DemolitionCalculation> updateCalculations = new LinkedList<>();
		...
		if (!CollectionUtils.isEmpty(calculations)) {
			...
			Set<String> applicationNos = calculations.stream()
					.map(calculation -> calculation.getDemolition().getApplicationNo()).collect(Collectors.toSet());
//            Set<String> applicationNumbers = calculations.stream().map(calculation -> calculation.getBPA().getApplicationNo()).collect(Collectors.toSet());
			...
			for (DemolitionCalculation calculation : calculations) {
				if (!applicationNumbersFromDemands.contains(calculation.getDemolition().getApplicationNo()))
	...
	private List<Demand> updateDemand(RequestInfo requestInfo, List<DemolitionCalculation> calculations) {

		List<Demand> demands = new LinkedList<>();
		for (DemolitionCalculation calculation : calculations) {

			List<Demand> searchResult = searchDemand(calculation.getTenantId(),
					Collections.singleton(calculation.getDemolition().getApplicationNo()), requestInfo, calculation);

			if (CollectionUtils.isEmpty(searchResult))
				throw new CustomException(BPACalculatorConstants.INVALID_UPDATE,
						"No demand exists for applicationNumber: " + calculation.getDemolition().getApplicationNo());
	...
	private List<DemandDetail> getUpdatedDemandDetails(DemolitionCalculation calculation,
	...
	private void addRoundOffTaxHead(@NotNull String tenantId, List<DemandDetail> demandDetails) {
	...
	private List<Demand> createDemand(RequestInfo requestInfo, List<DemolitionCalculation> calculations) {

		List<Demand> demands = new LinkedList<>();
		for (DemolitionCalculation calculation : calculations) {
			Demolition demolition = null;

			if (calculation.getDemolition() != null) {
				demolition = calculation.getDemolition();
			} else if (calculation.getApplicationNumber() != null) {
				demolition = demolitionService.getDemolition(requestInfo, calculation.getTenantId(),
			...
			if (demolition == null)
				throw new CustomException(BPACalculatorConstants.INVALID_APPLICATION_NUMBER,
						"Demand cannot be generated for applicationNumber " + calculation.getApplicationNumber()
								+ "  Demolition application with this number does not exist ");

			String tenantId = calculation.getTenantId();
			String consumerCode = calculation.getDemolition().getApplicationNo();

			User owner = demolition.getOwners().get(0).toCommonUser();
			...
			demands.add(Demand.builder().consumerCode(consumerCode).demandDetails(demandDetails).payer(owner)
					...
					.taxPeriodFrom(startCal.getTimeInMillis()).taxPeriodTo(endCal.getTimeInMillis()).consumerType("BD")
					.businessService(utils.getBillingBusinessService()).build());
		}
		return demandRepository.saveDemand(requestInfo, demands);
	}

	private List<Demand> searchDemand(String tenantId, Set<String> consumerCodes, RequestInfo requestInfo,
			DemolitionCalculation calculation) {

		String feeType = calculation.getFeeType();
		String uri = utils.getDemandSearchURL();
		uri = uri.replace("{1}", tenantId);
		uri = uri.replace("{2}", utils.getBillingBusinessService());
