...
//			calcValidator.validatePropertyForCalculation(detail);         // VALIDATION IS NOT REQUIRED BECAUSE AUTOCALCAULATION IS NOT HAPPENING PT SUJOG PROJECT
	...
	private void generateDemandsFroMutationFee(PropertyV2 property, Map<String, Calculation> feeStructure, RequestInfo requestInfo) {
		...
		for(String key: feeStructure.keySet()) {
			...
			Demand demand = Demand.builder().auditDetails(null).additionalDetails(null).businessService(configs.getPtMutationBusinessCode())
					...
					.taxPeriodFrom(calculation.getFromDate()).taxPeriodTo(calculation.getToDate()).tenantId(calculation.getTenantId())
					.billExpiryTime(2629800000L).build();
			demands.add(demand);
