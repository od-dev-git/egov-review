...
public class PropertyController {
	...
	@PostMapping("/_count")
	public ResponseEntity<PropertyResponse> count(@Valid @RequestBody RequestInfoWrapper requestInfoWrapper,
			@Valid @ModelAttribute PropertyCriteria propertyCriteria) {

		PropertyCount count = propertyService.countProperty(propertyCriteria, requestInfoWrapper.getRequestInfo());
		PropertyResponse response = PropertyResponse.builder().propertyCount(count).responseInfo(
				responseInfoFactory.createResponseInfoFromRequestInfo(requestInfoWrapper.getRequestInfo(), true))
				.build();
		return new ResponseEntity<>(response, HttpStatus.OK);
	}
    ...
    //For Legacy Search Tab
    @PostMapping("/_searchLegacy")
    public ResponseEntity<LegacyPropertyResponse> searchLegacy(@Valid @RequestBody RequestInfoWrapper requestInfoWrapper,
                                                   @Valid @ModelAttribute PropertyCriteria propertyCriteria) {
    	List<LegacyProperty> legacyProperty = propertyService.searchLegacyProperty(propertyCriteria,requestInfoWrapper.getRequestInfo());
        LegacyPropertyResponse response = LegacyPropertyResponse.builder().legacyProperty(legacyProperty).responseInfo(
        		responseInfoFactory.createResponseInfoFromRequestInfo(requestInfoWrapper.getRequestInfo(), true))
        		.build();
        return new ResponseEntity<>(response, HttpStatus.OK);
    }    
...
//	}
    ...
    @PostMapping("/_updateDDNNo")
    public ResponseEntity<DDNNoDetailResponse> updateDDNNo(@RequestBody DDNNoDetailRequest ddnNoDetailRequest){
        DDNNoDetail ddnNoDetail=propertyService.updateDDNNo(ddnNoDetailRequest);
        ResponseInfo resInfo = responseInfoFactory.createResponseInfoFromRequestInfo(ddnNoDetailRequest.getRequestInfo(), true);
        DDNNoDetailResponse ddnNoDetailResponse = DDNNoDetailResponse.builder()
                .ddnNoDetails(Arrays.asList(ddnNoDetail))
                .responseInfo(resInfo).build();
        return new ResponseEntity<>(ddnNoDetailResponse, HttpStatus.OK);
