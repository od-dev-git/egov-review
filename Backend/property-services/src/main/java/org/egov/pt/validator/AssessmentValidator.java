...
public class AssessmentValidator {
	...
	public void validateAssessmentCreate(AssessmentRequest assessmentRequest, Property property) {
		...
		validateExistingApplicationInWorkflow(assessmentRequest);
	...
	private void validateExistingApplicationInWorkflow(AssessmentRequest request) {
		if(!ObjectUtils.isEmpty(request.getAssessment().getPropertyId())) {
			PropertyCriteria criteria = PropertyCriteria.builder()
					.tenantId(request.getAssessment().getTenantId())
					.propertyIds(Collections.singleton(request.getAssessment().getPropertyId()))
					.build();
			List<Property> properties = propertyService.searchPropertyPlainSearch(criteria, request.getRequestInfo());
