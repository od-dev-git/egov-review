...
import org.egov.pt.models.Email;
...
public class NotificationUtil {
   ...
   public List<Event> enrichEvent(List<SMSRequest> smsRequests, RequestInfo requestInfo, String tenantId, Property property, Boolean isActionReq){
	...
	public List<EmailRequest> createEmailRequest(String message, Map<String, String> emailToOwners, PropertyRequest request, AssessmentRequest asmtRequest) {
		
		List<EmailRequest> emailRequests = new ArrayList<>();
		for (Map.Entry<String, String> entryset : emailToOwners.entrySet()) {
			
			if(request != null && asmtRequest == null) {
				
				String subject = config.getEmailSubject().replaceAll(PTConstants.EMAIL_SUBJECT_KEY, request.getProperty().getAcknowldgementNumber());
				Set<String> emailTo = new HashSet<>(Arrays.asList(entryset.getKey()));
				String customizedMsg = message.replace(NOTIFICATION_OWNERNAME, entryset.getValue());
				Email email = Email.builder().body(customizedMsg).emailTo(emailTo).subject(subject).build();
				emailRequests.add(EmailRequest.builder().email(email).requestInfo(request.getRequestInfo()).build());
			}
			if(request == null && asmtRequest != null) {
				
				String subject = config.getEmailSubject().replaceAll(PTConstants.EMAIL_SUBJECT_KEY, asmtRequest.getAssessment().getAssessmentNumber());
				Set<String> emailTo = new HashSet<>(Arrays.asList(entryset.getKey()));
				String customizedMsg = message.replace(NOTIFICATION_OWNERNAME, entryset.getValue());
				Email email = Email.builder().body(customizedMsg).emailTo(emailTo).subject(subject).build();
				emailRequests.add(EmailRequest.builder().email(email).requestInfo(asmtRequest.getRequestInfo()).build());
			}
		}
