...
public class NotificationService {
	...
	private String getMsgForUpdate(Property property, String msgCode, String completeMsgs, String createUpdateReplaceString) {
		...
		/* Shorten url to be replaced with Citizen landing...
		 * 
		...
		String url = configs.getHomePageUrl();
	...
	private String getLocalisedState(String state, String completeMsgs) {
		...
		case WF_STATUS_SENT_BACK_TO_CITIZEN:
			return notifUtil.getMessageTemplate(WF_STATUS_SENT_BACK_TO_CITIZEN_LOCALE, completeMsgs);
	...
	private void prepareMsgAndSend(PropertyRequest request, String msg, String state) {
		...
		if(emailToOwners.isEmpty()) {
			...
			if(msg.contains("ID#"))
				msg=msg.split("ID#")[0];
			List<EmailRequest> emailRequestList = notifUtil.createEmailRequest(msg, emailToOwners, request, null);
			notifUtil.sendEmail(emailRequestList);
		}
	}
