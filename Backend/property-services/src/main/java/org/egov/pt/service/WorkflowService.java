...
public class WorkflowService {
	...
	public List<ProcessInstance> getProcessInstanceForIssueFix(RequestInfo requestInfo, String applicationNo,
		...
		StringBuilder url = getProcessInstanceSearchURLIssueFix(tenantId, applicationNo,history);
		RequestInfoWrapper requestInfoWrapper = RequestInfoWrapper.builder().requestInfo(requestInfo).build();
		Optional<Object> optional = serviceRequestRepository.fetchResult(url, requestInfoWrapper);
		ProcessInstanceResponse response = null;
		try {
			response = mapper.convertValue(optional.get(), ProcessInstanceResponse.class);
		} catch (IllegalArgumentException e) {
	...
	private StringBuilder getProcessInstanceSearchURLIssueFix(String tenantId, String applicationNo, Boolean history) {
		StringBuilder url = new StringBuilder(configs.getWfHost());
		url.append(configs.getWfProcessInstanceSearchPath());
		url.append("?tenantId=");
		url.append(tenantId);
