...
public class EnrichmentService {
	...
	public void enrichMutationRequest(PropertyRequest request, Property propertyFromSearch) {
		...
		if (CreationReason.MUTATION.equals(request.getProperty().getCreationReason()) && "REOPEN".equalsIgnoreCase(property.getWorkflow().getAction())) {
			if (!CollectionUtils.isEmpty(property.getDocuments()))
				property.getDocuments().forEach(doc -> {
	...
	public void enrichActivationRequest(PropertyRequest request, Property propertyFromSearch) {
		...
		if(request.getProperty().getCreationReason().equals(CreationReason.DEACTIVATE)) {
			property.setCreationReason(CreationReason.DEACTIVATE);
			property.setStatus(Status.DEACTIVATED);
		}
		
		if(request.getProperty().getCreationReason().equals(CreationReason.REACTIVATE)) {
			property.setCreationReason(CreationReason.REACTIVATE);
			property.setStatus(Status.ACTIVE);
		}
		property.setWorkflow(null);
