...
public class DiffService {
    ...
    public List<String> getUpdatedFields(Object propertyFromUpdate, Object propertyFromSearch, String flowType) {
        ...
        normalizeAdditionalDetailsToJson(propertyFromUpdate);
        normalizeAdditionalDetailsToJson(propertyFromSearch);
    ...
    private void normalizeAdditionalDetailsToJson(Object property) {
        if (property == null) {
            return;
        }

        try {
            // Get all declared fields from the class and its superclasses (PropertyInfo and Property)
            Class<?> clazz = property.getClass();

            // Log fields in the class and superclasses
            while (clazz != null) {
                Field[] fields = clazz.getDeclaredFields();
                for (Field field : fields) {
                    log.info("Field name: " + field.getName() + ", Field type: " + field.getType());
                }
                clazz = clazz.getSuperclass();  // Move to the superclass to check its fields as well
            }

            Field addressField = (property instanceof Property) 
            	    ? property.getClass().getSuperclass().getDeclaredField("address") 
            	    : property.getClass().getDeclaredField("address");


            addressField.setAccessible(true); // Allows access to private fields
            Object address = addressField.get(property);

            if (address != null) {
                // Get the `additionalDetails` field inside `address`
                Field additionalDetailsField = address.getClass().getDeclaredField("additionalDetails");
                additionalDetailsField.setAccessible(true);
                Object additionalDetails = additionalDetailsField.get(address);
                ...
                if (additionalDetails instanceof Map) {
                    ...
                    additionalDetailsField.set(address, objectNode);
                    log.info("Converted Map to ObjectNode: " + objectNode.toString());
                } 
            }
        } catch (NoSuchFieldException | IllegalAccessException e) {
            throw new RuntimeException("Error normalizing additionalDetails field inside address", e);
        }
    }
