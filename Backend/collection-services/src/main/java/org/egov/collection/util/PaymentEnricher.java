...
import static org.egov.collection.model.enums.PaymentModeEnum.WALLET;
...
public class PaymentEnricher {
	...
	public void enrichPaymentPreValidate(PaymentRequest paymentRequest) {
		...
		//round off total amount paid here
		enrichPaymentAmount(payment);
	...
	public void enrichPaymentPostValidate(PaymentRequest paymentRequest) {
		...
		if (paymentMode.equalsIgnoreCase(ONLINE.name()) || paymentMode.equalsIgnoreCase(CARD.name()) ||
			...
			paymentMode.equalsIgnoreCase(WALLET.name()))
	...
	private void enrichPaymentAmount(Payment payment) {
		if(!(payment.getTotalAmountPaid().stripTrailingZeros().scale() <= 0)) {
			payment.setTotalAmountPaid(payment.getTotalAmountPaid().setScale(0, RoundingMode.HALF_DOWN));
			payment.getPaymentDetails().get(0).setTotalAmountPaid(payment.getPaymentDetails().get(0).getTotalAmountPaid().setScale(0, RoundingMode.HALF_DOWN));
		}
	}
