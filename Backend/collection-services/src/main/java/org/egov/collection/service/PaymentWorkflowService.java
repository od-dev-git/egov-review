...
public class PaymentWorkflowService {
	...
	public List<Payment> cancelReceipt(@Valid PaymentWorkflowRequest receiptWorkflowRequest) {
		
		PaymentWorkflow.PaymentAction action = receiptWorkflowRequest.getPaymentWorkflows().get(0).getAction();
        String tenantId = receiptWorkflowRequest.getPaymentWorkflows().get(0).getTenant();

        Set<String> paymentIds = new HashSet<>();
        Map<String, PaymentWorkflow> workflowRequestByPaymentId = new HashMap<>();

        for(PaymentWorkflow workflow : receiptWorkflowRequest.getPaymentWorkflows()){
            if(!workflow.getAction().equals(action))
                throw new CustomException("PAYMENT_WORKFLOW_SINGLE_ACTION_ALLOWED", "All workflow requests should be " +
                        "of the same action");
            if(!workflow.getTenant().equalsIgnoreCase(tenantId))
                throw new CustomException("CROSS_TENANT_OP_NOT_ALLOWED", "All requests should act on a single tenant ");
            paymentIds.add(workflow.getPaymentId());
            workflowRequestByPaymentId.put(workflow.getPaymentId(), workflow);
        }
     // Fetch consumer codes of receipts
        ...
        switch (action){
            case CANCEL:
                processedPayments = cancelReceipt(workflowRequestByPaymentId, consumerCodes,
                		receiptWorkflowRequest.getRequestInfo(), tenantId);
                break;
            case DISHONOUR:
                processedPayments = dishonourReceipt(workflowRequestByPaymentId, consumerCodes,
                		receiptWorkflowRequest.getRequestInfo(), tenantId);
                break;
        }
	...
	private List<Payment> dishonourReceipt(Map<String, PaymentWorkflow> workflowRequestByPaymentId,
			Set<String> consumerCodes, RequestInfo requestInfo, String tenantId) {
		...
		PaymentSearchCriteria paymentSearchCriteria = PaymentSearchCriteria.builder().consumerCodes(consumerCodes)
				.paymentModes(paymentModes).instrumentStatus(status).tenantId(tenantId).offset(0)
				.limit(applicationProperties.getReceiptsSearchDefaultLimit()).build();

		List<Payment> payments = paymentRepository.fetchPayments(paymentSearchCriteria);
		payments.sort(reverseOrder(Comparator.comparingLong(Payment::getTransactionDate)));

		List<Payment> validatedPayments = paymentWorkflowValidator
				.validateForCancel(new ArrayList<>(workflowRequestByPaymentId.values()), payments);
		...
		validatedPayments.forEach(payment -> {
			collectionProducer.producer(applicationProperties.getCancelPaymentTopicName(),
					new PaymentRequest(requestInfo, payment));
		});

		return validatedPayments;
	}

	private List<Payment> cancelReceipt(Map<String, PaymentWorkflow> workflowRequestByPaymentId,
			Set<String> consumerCodes, RequestInfo requestInfo, String tenantId) {
		PaymentSearchCriteria paymentSearchCriteria = PaymentSearchCriteria.builder().consumerCodes(consumerCodes)
				.instrumentStatus(new HashSet<String>(Arrays.asList(InstrumentStatusEnum.APPROVED.toString(),
						InstrumentStatusEnum.REMITTED.toString())))
				.offset(0).limit(applicationProperties.getReceiptsSearchDefaultLimit()).tenantId(tenantId).build();
